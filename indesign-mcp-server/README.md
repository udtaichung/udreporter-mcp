...
- Adobe InDesign 2025 desktop (launched at least once)
- Node.js 18+
- Windows 10/11 (COM via winax) or macOS (AppleScript)
...
### Installation
  ...
  3. Start the server:
   ...
   - Windows (recommended): use the provided batch launcher so paths resolve cleanly
...
### MCP client config (Windows)

Use `mcp-config-with-batch.json` to launch via the batch file:

```json
{
  "mcpServers": {
    "indesign-mcp-server": {
      "command": "F:/MCP/indesign mcp/start-indesign-mcp.bat",
      "args": [],
      "env": {}
    }
  }
...
Alternatively, point directly to `src/index.js` (ensure the path matches your folder layout):
...
```json
{
  "mcpServers": {
    "indesign-mcp-server": {
      "command": "node",
      "args": ["F:/MCP/indesign mcp/indesign-mcp-server/src/index.js"],
      "env": {}
    }
  }
}
...
Note: The included `mcp-config-fixed.json` has an example path that uses a different folder name (`indesign-mcp`). Adjust paths to your actual directory (`indesign mcp`).
